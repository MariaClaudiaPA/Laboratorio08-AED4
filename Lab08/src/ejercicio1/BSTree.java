...
public class BSTree<E extends Comparable<E>> implements LinkBST<E> {
    ...
    public E search(E x) throws ItemNoFound {
        Node actual = root;
        while (actual != null) {
            int res = x.compareTo(actual.data);
            if (res == 0) {
                return actual.data;
            } else if (res < 0) {
                actual = actual.left;
    ...
    private Node remove(Node root, E x) throws ItemNoFound {
        ...
        if (actual.left == null && actual.right == null) {  // caso 1: nodo con cero hijos (hoja)
            if (actual == root) {
                return null;
            } else if (isLeftChild) {
                padre.left = null;
        ...
        } else if (actual.right == null) { // caso 2: nodo con un solo hijo
            if (actual == root) {
                return actual.left;
            } else if (isLeftChild) {
    ...
    public int areaBST() throws ExceptionIsEmpty {
        ...
        int height = height();
        int hojaCont = contarHojas(root);
        return height * hojaCont;
    }
